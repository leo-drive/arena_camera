...
public:
  ...
  explicit TimeKeeperSequental(std::string name)
  : name_(std::move(name)){
  ...
  std::string StrLog()
  {
    ...
    for (int i = 0; i < vec_timepoints_.size(); ++i) {
      str_log += name_ + " - " + vec_timepoint_names_[i] + " took " +
                 std::to_string(MillisPassedBetween(vec_timepoints_[i - 1], vec_timepoints_[i])) +
                 " ms.\n";
    }

    str_log +=
      name_ + " - Total time: " +
      std::to_string(MillisPassedBetween(vec_timepoints_.front(), vec_timepoints_.back())) +
      " ms.\n";
  ...
  void PrintTimes()
  {
    for (int i = 1; i < vec_timepoints_.size(); ++i) {
      std::cout << name_ << " - " << vec_timepoint_names_[i] << " took "
                << MillisPassedBetween(vec_timepoints_[i - 1], vec_timepoints_[i]) << " ms."
                << std::endl;
    }

    std::cout << name_ << " took TOTAL "
              << MillisPassedBetween(vec_timepoints_.front(), vec_timepoints_.back()) << " ms."
              << std::endl;
  };
